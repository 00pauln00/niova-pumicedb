...
#define PMDB_ARG_CHECK(op, rncr)                     \
    NIOVA_ASSERT(                                    \
        ...
        (((char *)(rncr)->rncr_request->rcrm_data == \
          (rncr)->rncr_request_or_commit_data) ||    \
         op == RAFT_NET_CLIENT_REQ_TYPE_COMMIT) &&   \
        (rncr)->rncr_reply.rncr_reply_data_max_size >= sizeof(struct pmdb_msg))
...
reply:
    ...
    PMDB_OBJ_DEBUG((pmdb_reply->pmdbrm_err == -EBADE ? LL_NOTIFY : LL_DEBUG),
                   &obj, "op-err=%s new-object=%s (ppt=%ld)",
                   strerror(-pmdb_reply->pmdbrm_err), new_object ? "yes" : "no",
                   prev_pending_term);
...
pmdb_sm_handler_client_read(struct raft_net_client_request_handle *rncr)
{
    ...
    struct pmdb_object obj = {0};
    ssize_t rc = pmdb_object_lookup(&pmdb_req->pmdbrm_user_id, &obj,
                                    rncr->rncr_current_term);
    ...
    if (!rc || pmdb_rncui_is_read_any(&pmdb_req->pmdbrm_user_id))
    {
        ...
        rc = pmdbApi->pmdb_read(&read_cb_args);
    }
    ...
    if (rc < 0)
    {
        ...
        DBG_RAFT_CLIENT_RPC(LL_NOTIFY, req,
                            "pmdbApi::read(): %s", strerror(rc));
    }
    else if (rc > (ssize_t)max_reply_size)
    {
        pmdb_reply->pmdbrm_data_size = (uint32_t)rc;
        rc = -E2BIG;

        DBG_RAFT_CLIENT_RPC(LL_NOTIFY, req,
                            "pmdbApi::read(): reply too large (%zd)", rc);
    }
    else
    {
...
pmdb_sm_handler_client_rw_op(struct raft_net_client_request_handle *rncr)
{
    NIOVA_ASSERT(rncr && rncr->rncr_type == RAFT_NET_CLIENT_REQ_TYPE_NONE &&
                 rncr->rncr_request && rncr->rncr_reply.rncr_reply_ptr &&
                 rncr->rncr_request_or_commit_data_size >=
                 sizeof(struct pmdb_msg));
                              ...
                              struct raft_net_client_request_handle *rncr)
{
    ...
    rc = pmdbApi->pmdb_apply(&apply_args);
    if (rncr->rncr_is_leader)
    {
        // Add the reply size to the RPC reply
...
pmdb_sm_handler(struct raft_net_client_request_handle *rncr)
{
    ...
    {
        if (rncr->rncr_reply.rncr_reply_data_max_size < sizeof(struct pmdb_msg))
            return -ENOSPC;
 ...
 * context only.  This call is used by the application to stage deletion from
 * RocksDB.
 * @app_id:  identifier of the application instance
 * @pmdb_handle:  the handle which was provided from pumice_db to the apply
 *    callback.
...
PmdbDeleteKV(const struct raft_net_client_user_id *app_id, void *pmdb_handle,
            const char *key, size_t key_len, void (*comp_cb)(void *), void *app_handle)
{
    struct pmdb_apply_handle *pah = (struct pmdb_apply_handle *)pmdb_handle;
