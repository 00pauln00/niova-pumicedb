...
static bool use_logical_commands = false;
 ...
 pmdbtc_print_help(const int error, char **argv)
 {
    fprintf(error ? stderr : stdout,
            "Usage: %s [-a (use async requests)] [-l (enable logical commands)] -r <UUID> -u <UUID>\n",
            argv[0]);
 ...
 main(int argc, char **argv)
 {
    ...
    if (use_logical_commands) {
        SIMPLE_LOG_MSG(LL_WARN, "LOGICAL_CMD: Client started with logical commands enabled");
    }
